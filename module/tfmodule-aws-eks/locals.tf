locals {
  ...
  worker_iam_role_name     =  format("%sWorkerEC2Role", var.context.project)
  worker_iam_profile_name  =  format("%sWorkerEC2Profile", var.context.project)
  ...
  kubeconfig = var.create_eks ? templatefile("${path.module}/templates/kubeconfig.tpl", {
    kubeconfig_name                         = coalesce(var.kubeconfig_name, local.cluster_name)
