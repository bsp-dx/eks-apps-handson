resource "aws_cloudwatch_log_group" "this" {
  ...
  tags = merge(local.tags, { Name = "/aws/eks/${local.cluster_name}/cluster" })
}
...
resource "aws_eks_cluster" "this" {
  ...
  tags = merge( local.tags, var.cluster_tags, {Name = local.cluster_name})
...
resource "aws_iam_role" "cluster" {
  count = var.create_eks ? 1 : 0
  name                  = format("%sClusterRole", var.context.project)
  assume_role_policy    = data.aws_iam_policy_document.cluster_assume_role_policy.json
  permissions_boundary  = var.permissions_boundary
  path                  = var.iam_path
  force_detach_policies = true

  tags = merge(local.tags, { Name = format("%sClusterRole", var.context.project) })
}
...
resource "aws_iam_policy" "cluster_elb_sl_role_creation" {
  ...
  name = "${local.project}ELBServiceLinkPolicy"
...
resource "aws_iam_policy" "cluster_deny_log_group" {
  ...
  name        = "${local.project}EKSDenyLogGroupPolicy"
