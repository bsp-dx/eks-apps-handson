locals {
  ...
  map_roles_admin = length(var.iam_admin_role_arn) > 1 ? [{rolearn=var.iam_admin_role_arn, username="admin", groups=["system:masters"]}] : []
  map_roles_viewer = length(var.iam_viewer_role_arn) > 1 ? [{rolearn=var.iam_viewer_role_arn, username="viewer", groups=["system:viewers"]}] : []
}
  ...
  data = {
    mapRoles = yamlencode(
      distinct(concat(
        ...
        local.map_roles_admin,
        local.map_roles_viewer,
        var.map_roles,
      ))
    )
