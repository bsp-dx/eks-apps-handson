...
resource "aws_autoscaling_group" "workers" {
  ...
  dynamic "tag" {
    for_each = concat(
      ...
      lookup(var.worker_groups[count.index], "tags", local.workers_group_defaults["tags"])
    )
    content {
      key                 = tag.value.key
...
resource "aws_security_group_rule" "workers_ingress_cluster_kubelet" {
  count =  var.create_eks ? var.worker_sg_ingress_from_port > 10250 ? 1 : 0 : 0
...
resource "aws_iam_role" "workers" {
  ...
  tags = merge(local.tags, { Name = local.worker_iam_role_name })
}

resource "aws_iam_instance_profile" "workers" {
  count = var.create_eks ? local.worker_group_launch_configuration_count : 0

  name_prefix = local.worker_iam_profile_name
