  ...
  dynamic "tag" {
    for_each = concat(
      [
        {
          key     = "Name"
          value   = "${local.cluster_name}-${lookup(var.worker_groups_launch_template[count.index], "name", count.index)}-asg"
      ...
      lookup(var.worker_groups_launch_template[count.index], "tags", local.workers_group_defaults["tags"])
    )
    content {
      key                 = tag.value.key
...
resource "aws_launch_template" "workers_launch_template" {
  ...
  iam_instance_profile {
    name = coalescelist( aws_iam_instance_profile.workers_launch_template.*.name,)[count.index]
  }
  ...
  tag_specifications {
    ...
    tags = merge(
      {
        "Name" = "${local.cluster_name}-${lookup( var.worker_groups_launch_template[count.index], "name", count.index,)}-asg"
      },
      local.tags,
      {
